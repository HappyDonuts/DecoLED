...
power_handle_t* power_handle_new(RTC_HandleTypeDef *hrtc, UART_HandleTypeDef *huart)
{
	...
	power_handle_init(power_handle, hrtc, huart);
	return power_handle;
}
...
{
	if (power_handle->huart != NULL) {
		char buffer[21];

		HAL_RTC_GetTime(power_handle->hrtc, &sTime, RTC_FORMAT_BCD);
		HAL_RTC_GetDate(power_handle->hrtc, &sDate, RTC_FORMAT_BCD);
		...
		HAL_UART_Transmit(power_handle->huart, (uint8_t*)buffer, 21, 100);
		HAL_Delay(10);
	}
}
